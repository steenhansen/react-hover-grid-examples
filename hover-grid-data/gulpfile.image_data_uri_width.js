...
function clear_tile_sized_1 () {
  ...
  let tasks = image_folders.map(function (image_folder) {
    const tile_pictures = path.join(images_src, image_folder, '/' + TILE_SIZED + '/*')
    return gulp.src(tile_pictures)
      .pipe(vinylPaths(del))
  })
  return merge(tasks)
}

function resize_original_to_tile_size_2 () {
  const images_src = getPublicImages()
  const image_folders = getFolders(images_src)
  let tasks = image_folders.map(function (image_folder) {
...
function clear_low_quality_3 () {
  ...
  let tasks = image_folders.map(function (image_folder) {
    ...
    if (!gulp_ssr_low_quality_perc) {
      ...
      return gulp.src(small_pictures)
        .pipe(vinylPaths(del))
    }
  })
  return merge(tasks)
...
function tile_size_to_low_quality_small_4 () {
  const images_src = getPublicImages()
  const image_folders = getFolders(images_src)
  let tasks = image_folders.map(function (image_folder) {
    const data_folder = image_folder.replace('_images', '_data')
...
gulp.task('image_data_uri_width', gulp.series(
  clear_tile_sized_1
  , resize_original_to_tile_size_2
  , clear_low_quality_3
  , tile_size_to_low_quality_small_4
  , tiny_css_image_5
  , tile_width_height_6
  , fini_message_7
))
